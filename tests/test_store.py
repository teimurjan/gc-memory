...
class TestRetrieve:
    def test_top_k(self, config: Config, rng: np.random.Generator) -> None:
        entries = [
            _make_entry("a", _basis_vector(0), rng),
            _make_entry("b", _basis_vector(1), rng),
        ]
        store = GCMemoryStore(entries, config, rng)
        results = store.retrieve(_basis_vector(0), "test", k=2)
    ...
    def test_apoptotic_excluded(self, config: Config, rng: np.random.Generator) -> None:
        ...
        store = GCMemoryStore(entries, config, rng)
        ids = [e.id for e, _ in store.retrieve(_basis_vector(0), "test", k=10)]
        assert "dead" not in ids

    def test_empty(self, config: Config, rng: np.random.Generator) -> None:
...
class TestTierTransitions:
    def test_naive_to_gc(self, config: Config, rng: np.random.Generator) -> None:
        ...
        q = _basis_vector(0)
        retrieved = store.retrieve(q, "test", k=1)
        store.update_after_retrieval(q, "test", retrieved, step=0)
...
class TestUtilities:
    def test_get_all(self, config: Config, rng: np.random.Generator) -> None:
        entries = [
            _make_entry("a", _basis_vector(0), rng),
    ...
    def test_get_active(self, config: Config, rng: np.random.Generator) -> None:
        entries = [
            _make_entry("a", _basis_vector(0), rng),
            _make_entry("b", _basis_vector(1), rng, tier=Tier.APOPTOTIC),
        ]
