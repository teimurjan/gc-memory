...
Default config for the production row: `alpha=0.3`, `suppression_rate=0.1`, `reinforcement_rate=0.05`, `decay_lambda=0.005`, `n_clusters=30`, `use_rank_gap=True`. Reproducer: `research_playground/rif/run_clustered.py`. Stat-significance tests (paired permutation, bootstrap CI) from checkpoint 18 were on the old tokenizer; results with the new tokenizer have not been re-verified for significance.
...
# Retrieval-only
uv run python research_playground/baseline/run.py             # checkpoint 10 baselines
uv run python research_playground/rif/run.py                  # checkpoint 11 (global RIF)
uv run python research_playground/rif/run_clustered.py        # checkpoint 12
uv run python research_playground/rif/run_gap.py              # checkpoint 13 (best retrieval-only)
uv run python research_playground/rif/run_extended_metrics.py # checkpoint 16

# LLM enrichment (needs ANTHROPIC_API_KEY)
export ANTHROPIC_API_KEY=sk-ant-...
uv run python experiments/enrich_longmemeval.py    # ~$16 for 10k entries
uv run python research_playground/rif/run_enriched.py         # checkpoint 17 (3-arm)

# Statistical rigor + cross-dataset scope (checkpoint 18)
uv run python research_playground/rif/bootstrap_ci.py         # LongMemEval CIs + perm tests
uv run python research_playground/rif/run_gap_nfcorpus.py     # NFCorpus replication (~25 min)
...
Raw per-run outputs from each benchmark script are checked into `research_playground/<topic>/results/` for auditability.
