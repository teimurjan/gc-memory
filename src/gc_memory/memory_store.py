...
class MemoryStore:
    ...
    def retrieve(
        ...
        Pipeline: BM25 + FAISS hybrid → cross-encoder rerank → adaptive depth.
        """
        if self.bi_encoder is None:
        ...
        # Shallow pass: BM25 + vector hybrid
        candidate_ids = self.index.search_hybrid(query_emb, query, self.k_shallow)
        candidates = [
            (eid, self.entries[eid].content)
            for eid in candidate_ids if eid in self.entries
        ]
        ...
        if self.reranker.needs_deep_search(scores):
            deep_ids = self.index.search_hybrid(query_emb, query, self.k_deep)
            already = set(candidate_ids)
            new_ids = [eid for eid in deep_ids if eid not in already and eid in self.entries]
            if new_ids:
        ...
        for eid, score in scored[:k]:
            if eid in self.entries:
                ...
                # Affinity EMA from normalized xenc score
